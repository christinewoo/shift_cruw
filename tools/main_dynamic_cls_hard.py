    ...
    for pid, pair in zip(kept_dt_ids, cur_idx_pairs):
        ...
        if cls == 'Car':
            ...
            if iou < adj_iou_thresh[cls]:
                ...
                new_obj = copy.deepcopy(pair[1])
                if cls == 'Pedestrian':
                    depth_error = np.random.uniform(iou+0.4, iou+0.49, size=(1,))
                    error = np.random.uniform(iou+0.37, iou+0.48, size=(2,))
                    global pedA
                    pedA += 1
                    depth_error = np.around(depth_error / 0.01) * 0.01
                    depth_error = np.clip(depth_error, iou, 0.93)
                else: #'Cyclist'
                    depth_error = np.random.uniform(iou+0.38, iou+0.46, size=(1,))
                    error = np.random.uniform(iou+0.37, iou+0.48, size=(2,))
                    global cycA
                    cycA += 1
                    depth_error = np.around(depth_error / 0.01) * 0.01
                    depth_error = np.clip(depth_error, iou, 0.9)
                error = np.around(error / 0.01) * 0.01
                error = np.clip(error, iou, 0.9)
                error = sorted(error)

    ...
    for pid, pair in zip(kept_dt_ids, cur_idx_pairs):
        ...
        if cls == 'Car':
            if abs(pair[0].box2d[1]-pair[0].box2d[3]) < 35:
                ...
                depth_error = np.random.uniform(0.54, 0.68, size=(1,))
                ...
                tag = np.random.choice([True, False], p=[0.3, 0.7]) #0.7/0.3
                new_obj = copy.deepcopy(pair[1])
                depth_error = np.random.uniform(0.34, 0.42, size=(1,))
                error = np.random.uniform(0.42, 0.53, size=(2,))
                depth_error = np.around(depth_error / 0.01) * 0.01
                depth_error = np.clip(depth_error, iou, 0.79)
                error = np.around(error / 0.01) * 0.01
                error = np.clip(error, iou, 0.88)
                error = sorted(error)

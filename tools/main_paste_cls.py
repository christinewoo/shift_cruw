...
def get_gt_dt_pairs(datum, frame, cls='Car'):
    ...
    added = False
    # added = np.random.choice([True, False], p=[0.7, 0.3])
    for i in range(n):
        # if abs(gt_objs[i].box2d[1]-gt_objs[i].box2d[3]) <= 25:
        #     print(abs(gt_objs[i].box2d[1]-gt_objs[i].box2d[3]))
        matched_dt_idx = x[i]
        if matched_dt_idx != -1:
            ...
            # Check the score
            if gt_objs[i].score >= 0.5 and abs(gt_objs[i].box2d[1]-gt_objs[i].box2d[3]) <= 25 and not added: #gt_objs[i].score >= 0.5 and 
...
def fix_obj(cur_idx_pairs, kept_dt_ids, cls='Car'):
    ...
    for pid, pair in zip(kept_dt_ids, cur_idx_pairs):
        ...
        if cls == 'Pedestrian' or cls == 'Cyclist':
            ...
            if cls == 'Pedestrian':
                depth_error = np.random.uniform(0.09, 0.16, size=(1,))
                error = np.random.uniform(0.1, 0.17, size=(2,))
                global pedA
                pedA += 1
            else: # Cyclist
                depth_error = np.random.uniform(0.06, 0.12, size=(1,))
                error = np.random.uniform(0.1, 0.15, size=(2,))
                global cycA
                cycA += 1
            # Size the error
...
def main():
    ...
    for frame in tqdm(datum['gt'].keys()):
        ...
        for j in car_mods.keys():
            if j < 0:
                keep.append(car_mods[j].src)
                carB += 1
        for j in ped_mods.keys():
            if j < 0:
                keep.append(ped_mods[j].src)
                pedB += 1
        for j in cyc_mods.keys():
            if j < 0:
                keep.append(cyc_mods[j].src)
                pedB += 1
    ...
    print(f'Car: {carB},  Ped: {pedB},  Cyc: {cycB}')

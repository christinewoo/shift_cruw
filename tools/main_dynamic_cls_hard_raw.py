...
def fix_obj(cur_idx_pairs, kept_dt_ids, cls='Car'):
    ...
    for pid, pair in zip(kept_dt_ids, cur_idx_pairs):
        ...
        if iou == 0:
            continue
        if cls == 'Car':
            if abs(pair[0].box2d[1]-pair[0].box2d[3]) < 35:
                new_obj = copy.deepcopy(pair[1])
                depth_error = np.random.uniform(0.54, 0.62, size=(1,))
                error = np.random.uniform(0.42, 0.56, size=(2,))
                depth_error = np.around(depth_error / 0.01) * 0.01
                depth_error = np.clip(depth_error, iou, 0.92)
                error = np.around(error / 0.01) * 0.01
                error = np.clip(error, iou, 0.88)
                error = sorted(error)
                ...
                # DEBUG
                new_iou = get_iou(pair[0], new_obj, cls=cls)
                if new_iou == -1 or new_iou < iou: #
                    mod_objs.update({pid : copy.deepcopy(pair[1])})
                else:
                    carA += 1
            elif iou < adj_iou_thresh[cls]:
                tag = np.random.choice([True, False], p=[0.31, 0.69]) #0.7/0.3
                new_obj = copy.deepcopy(pair[1])
                depth_error = np.random.uniform(0.17, 0.25, size=(1,))
                error = np.random.uniform(0.32, 0.4, size=(2,))
                depth_error = np.around(depth_error / 0.01) * 0.01
                depth_error = np.clip(depth_error, iou, 0.75)
                error = np.around(error / 0.01) * 0.01
                error = np.clip(error, iou, 0.94)
                error = sorted(error)
                if tag: ### Adjust z-axis ###
                    ...
                    ### Adjust x-axis ###
                    if pair[0].loc[2] > pair[1].loc[2]: # gt.x > pd.x
                    ...
                    if pair[0].loc[0] > pair[1].loc[0]: # gt.z > pd.z
                        ...
                        new_obj.loc[0] = pair[1].loc[0] - abs(pair[1].loc[0] - pair[0].loc[0]) * depth_error
                    # if pair[0].loc[1] > pair[1].loc[1]: # gt.z > pd.z
                    #     new_obj.loc[2] = pair[1].loc[1] + abs(pair[1].loc[1] - pair[0].loc[1]) * error[0]
                    # else: # gt.z < pd.z
                    #     new_obj.loc[1] = pair[1].loc[1] - abs(pair[1].loc[1] - pair[0].loc[1]) * error[0]
                # Update new object        
                new_obj.src = new_obj.to_kitti_result()
            ...
            if iou < adj_iou_thresh[cls]:
                tag = np.random.choice([True, False], p=[0.6, 0.4]) #0.7/0.3
                new_obj = copy.deepcopy(pair[1])
                if cls == 'Pedestrian':
                    depth_error = np.random.uniform(iou+0.35, iou+0.42, size=(1,))
                    error = np.random.uniform(iou+0.33, iou+0.40, size=(2,))
                    global pedA
                    pedA += 1
                else: #'Cyclist'
                    depth_error = np.random.uniform(iou+0.35, iou+0.42, size=(1,))

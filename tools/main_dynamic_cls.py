...
def fix_obj(cur_idx_pairs, kept_dt_ids, cls='Car'):
    ...
    for pid, pair in zip(kept_dt_ids, cur_idx_pairs):
        ...
        if iou < adj_iou_thresh[cls]:
            ...
            if cls == 'Car':
                depth_error = np.random.uniform(iou+0.27, iou+0.34, size=(1,))
                error = np.random.uniform(iou+0.21, iou+0.28, size=(2,))
            else:
                depth_error = np.random.uniform(iou+0.17, iou+0.25, size=(1,))
